...
def calculate_black_pixels_percentage(image):
    ...
    percentage = (100.00 - float(percentage)) * .075
...
class VideoProcessor:
   ...
   "leclerc-china": {
    ...
    def extract_frames(self, start_frame: int, end_frame: int, fps_target: int = 10) -> List[np.ndarray]:
        ...
        # Inicializar SAM
        sam_checkpoint = r"models/sam_vit_b_01ec64.pth"  # Ruta al archivo del modelo descargado
        model_type = "vit_b"
        ...
        def mask_helmet_sam(color_image: np.ndarray, helmet_height_ratio: float = 0.3, prev_mask: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray]:
            """
            Usa Segment Anything Model (SAM) para segmentar el casco y lo pinta de verde.
            ...
            if prev_mask is not None:
                ...
                # Convertir la imagen a RGB (SAM espera imágenes en RGB)
                image_rgb = cv2.cvtColor(color_image, cv2.COLOR_BGR2RGB)
                
                # Configurar la imagen en el predictor de SAM
                predictor.set_image(image_rgb)
                
                # Definir una caja delimitadora (bounding box) aproximada para el casco
                roi_height = int(height * helmet_height_ratio)  # Altura de la ROI
                roi_width = int(width)  # Ancho de la ROI (100% del ancho de la imagen)
                roi_x = (width - roi_width) // 2  # Centrar horizontalmente
                roi_y = height - roi_height  # Parte inferior
                input_box = np.array([roi_x, roi_y, roi_x + roi_width, height])  # [x1, y1, x2, y2]
                
                # Segmentar el casco con SAM
                masks, scores, _ = predictor.predict(
                    box=input_box[None, :],  # Proporcionar la caja delimitadora
                    multimask_output=False  # Solo queremos una máscara   
                )

                # Seleccionar la máscara con mayor confianza
                best_mask = masks[np.argmax(scores)]  # Máscara con el puntaje más alto
                mask_final = (best_mask * 255).astype(np.uint8)  # Convertir a formato uint8 (0 o 255)
            ...
            result_original = cv2.bitwise_and(result, result, mask=mask_inv)
